...
## A20. First graded PASS, and the guard that passed without working — 2026-09-20

**Aethar, controlling breaker, v6 `0b229539…`: all five frozen questions HOLD, exit 0.** First
graded `--all` that does not fail Q4 or A6. Transcript unedited at
`agent_outputs/harness-graded-v6-2026-09-20-aethar/`. 17:29:20–17:30:59 EDT, stderr empty.

**Bounded exactly as he bounded it:** this run, `gemini-3.6-flash`, billed key. Not a win, not a
submission, and not general reliability. Q2 is also true live — `rev-list --count` still 1 — which
is luck, not knowledge, and the answer says so.

### Remainder 1, fixed: R7 on KB was a request, not a control

A19 required KB answers to cite entry path **AND** the kb id. v6 shipped:

```python
if KB not in src_text and "/" not in src_text:   # passes if EITHER is present
```

That is OR. **The rule read as enforced and was not** — it only survived because the model
volunteered both. Aethar's words: *a request, not a control*, which is the exact failure this
project's doctrine names.

v7 checks each independently and proves it bites:

```
path only            -> ['no kb id']
kb id only           -> ['no entry path']
both (v6 answers)    -> ['clean']
neither              -> ['no entry path', 'no kb id']
```

**The v6 PASS stands as recorded against v6.** It does not transfer to v7. v7 is strictly stricter
— it cannot turn a v6 violation into a v6 pass — but a verdict on v7 requires a run on v7.

### Remainder 2, recorded not fixed

`route()` sends any `claim-*` token to GROQ, including prose *about* an id rather than a request
for its fields. The frozen five are unaffected. Recorded as a known limit rather than patched
mid-verdict.

Candidate v7 SHA-256: `330a94ab04b127780cf91f2d70e254af45f292279ead4821afd672019b1dfbfd`.
v6 preserved at `ask.py.v6`, v5 at `ask.py.v5`.

**Next brick is the DEV post, per the breaker. Not another harness pass.** Rotate the `…7Tow` key
before it ships.
